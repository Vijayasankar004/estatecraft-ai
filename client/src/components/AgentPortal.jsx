...
export default function AgentPortal({ 
  inventory = [],
  onSaveListing, 
  onRemovePhotoFromListing,
  onAddPhotosToListing,
  onDeleteListing,
  existingInventoryCount = 0, 
  onSwitchToBuyer, 
  currentUser, 
  onOpenAuth,
  language = 'en',
  ...
  // Navigation section: 'create' | 'stored_manager'
  const [activeSection, setActiveSection] = useState('create');
  const [storedFilter, setStoredFilter] = useState('all'); // 'all' | 'mine'
  const [storedSearchQuery, setStoredSearchQuery] = useState('');
  const [storedPhotoIdxMap, setStoredPhotoIdxMap] = useState({});
  const [permissionAlert, setPermissionAlert] = useState(null);
  const [photoActionFeedback, setPhotoActionFeedback] = useState(null);
  const [newPhotoUrlMap, setNewPhotoUrlMap] = useState({});
  ...
  const handleSaveToInventory = () => {
    ...
    const newListing = {
      ...
      agentEmail: currentUser?.email || 'vikram@sothebysrealty.in',
  ...
  // Helper to check if currently logged-in user is the specific owner agent
  const isAgentOwner = (property) => {
    if (!currentUser) return false;
    if (property.agentId && currentUser.id === property.agentId) return true;
    if (property.agentEmail && currentUser.email?.toLowerCase() === property.agentEmail?.toLowerCase()) return true;
    if (!property.agentId && !property.agentEmail && currentUser.role === 'agent') return true;
    return false;
  };

  // Handle Photo cycling in stored listing manager
  const handleStoredPrevPhoto = (listingId, total) => {
    setStoredPhotoIdxMap(prev => ({
      ...prev,
      [listingId]: ((prev[listingId] || 0) - 1 + total) % total
    }));
  };

  const handleStoredNextPhoto = (listingId, total) => {
    setStoredPhotoIdxMap(prev => ({
      ...prev,
  ...
  // Attempt to remove photo from stored listing (Authorization check)
  const handleAttemptRemoveStoredPhoto = async (property, photoIdx) => {
    if (!isAgentOwner(property)) {
      setPermissionAlert({
        title: 'Action Blocked: Agent Permission Required',
        message: `This property's stored photos can only be removed by the specific listing agent (${property.agentName || property.agentEmail || 'the original agent'}). You are currently active as ${currentUser?.name || 'Guest'}.`,
        authorizedAgent: property.agentName || 'Original Agent'
      });
      return;
    }

    const confirmed = window.confirm(`Are you sure you want to remove photo #${photoIdx + 1} from "${property.address}"? This change will be permanently saved.`);
    if (!confirmed) return;

    if (onRemovePhotoFromListing) {
      const res = await onRemovePhotoFromListing(property.id, photoIdx);
      if (res && res.error) {
        setPhotoActionFeedback({ type: 'error', message: res.error });
      } else {
        setPhotoActionFeedback({
          type: 'success',
          message: `Photo successfully removed from "${property.address}" by authorized agent (${currentUser.name}).`
        });
        setStoredPhotoIdxMap(prev => ({
          ...prev,
          [property.id]: Math.max(0, (prev[property.id] || 0) - 1)
        }));
      }
      setTimeout(() => setPhotoActionFeedback(null), 4000);
    }
  };

  // Attempt to add new photo to stored listing (Authorization check)
  const handleAttemptAddStoredPhoto = async (property, photoUrlToAdd = null) => {
    if (!isAgentOwner(property)) {
      setPermissionAlert({
        title: 'Action Blocked: Agent Permission Required',
        message: `Only the specific listing agent (${property.agentName || property.agentEmail || 'the original agent'}) can add photos to this property.`,
        authorizedAgent: property.agentName || 'Original Agent'
      });
      return;
    }

    const url = (photoUrlToAdd || newPhotoUrlMap[property.id] || '').trim();
    if (!url) return;

    if (onAddPhotosToListing) {
      const res = await onAddPhotosToListing(property.id, [url]);
      if (res && res.error) {
        setPhotoActionFeedback({ type: 'error', message: res.error });
      } else {
        setPhotoActionFeedback({
          type: 'success',
          message: `New photo added to "${property.address}" by authorized agent (${currentUser.name}).`
        });
        setNewPhotoUrlMap(prev => ({ ...prev, [property.id]: '' }));
      }
      setTimeout(() => setPhotoActionFeedback(null), 4000);
    }
  };
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-16 transition-colors duration-200">
      ...
      {photoActionFeedback && (
        <div className={`p-4 rounded-2xl border text-xs flex items-center justify-between gap-2 shadow-lg animate-fadeIn ${
          photoActionFeedback.type === 'error'
          ...
          <div className="flex items-center gap-2">
            ...
            <span>{photoActionFeedback.message}</span>
          </div>
          <button
            type="button"
            onClick={() => setPhotoActionFeedback(null)}
      ...
      {/* View Switcher: 1. AI Studio & Create Listing vs 2. Stored Properties & Photo Gallery Manager */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b dark:border-slate-800 border-slate-200 pb-4">
        <div className="flex items-center gap-2 p-1 rounded-2xl dark:bg-slate-900 bg-slate-100 border dark:border-slate-800 border-slate-300">
          <button
            type="button"
          ...
          >
            ...
            <span>AI Listing Studio & Creator</span>
          </button>
          <button
            type="button"
            onClick={() => setActiveSection('stored_manager')}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
              activeSection === 'stored_manager'
                ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/30'
                : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
            }`}
          >
            <FolderOpen className="w-3.5 h-3.5" />
            <span>Stored Properties & Photos</span>
            <span className="px-2 py-0.5 rounded-full text-[10px] font-mono bg-slate-950/40 text-emerald-300 font-bold">
              {inventory.length}
            </span>
          </button>
        </div>

        {/* Current Agent Info Badge */}
        <div className="flex items-center gap-2 text-xs">
          <span className="text-slate-500 dark:text-slate-400">Authenticated Agent:</span>
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl dark:bg-slate-900 bg-slate-100 border dark:border-slate-800 border-slate-200 font-medium dark:text-slate-200 text-slate-800 shadow-sm">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="font-bold">{currentUser?.name || 'Vikram Malhotra'}</span>
            <span className="text-[10px] text-slate-500 font-mono hidden sm:inline">({currentUser?.email || 'vikram@sothebysrealty.in'})</span>
          </div>
        </div>
      </div>

      {/* Conditional Rendering: Section 1 (AI Studio) vs Section 2 (Stored Properties & Photo Manager) */}
      {activeSection === 'create' ? (
          ...
          <div className="glass-panel rounded-3xl p-6 border dark:border-slate-800 border-slate-200 shadow-xl lg:aspect-square flex flex-col justify-between overflow-hidden transition-all duration-200">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1.5 py-2 custom-scrollbar">
              ...
              <div className="space-y-3 p-3.5 rounded-2xl dark:bg-slate-900/80 bg-slate-100/90 border dark:border-slate-800 border-slate-200">
                ...
                {/* Specific Agent Storage & Removal Policy Notice */}
                <div className="flex items-center gap-2 p-2.5 rounded-xl dark:bg-emerald-950/40 bg-emerald-50 border dark:border-emerald-500/30 border-emerald-200 text-[11px] text-emerald-800 dark:text-emerald-300">
                  <ShieldCheck className="w-4 h-4 shrink-0 text-emerald-500" />
                  <span>
                    <strong>Storage & Security:</strong> Uploaded photos will be permanently stored and can <u>only be removed by your verified agent account</u> ({currentUser?.name || 'Vikram Malhotra'}).
                  </span>
                </div>
      ...
      /* Section 2: Stored Properties & Photo Gallery Manager */
      <div className="space-y-6 animate-fadeIn">
        {/* Sub-Header & Controls */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 rounded-3xl dark:bg-slate-900/60 bg-white border dark:border-slate-800 border-slate-200 shadow-sm">
          <div className="space-y-1">
            <h2 className="text-xl font-bold dark:text-white text-slate-900 flex items-center gap-2">
              <FolderOpen className="w-5 h-5 text-emerald-500" />
              Stored Inventory & Photo Manager
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Only the specific agent who uploaded a property can delete or modify its stored photos.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-2.5">
            {/* Filter Tabs: All vs Mine */}
            <div className="flex items-center p-1 rounded-xl dark:bg-slate-950 bg-slate-100 border dark:border-slate-800 border-slate-300 text-xs">
              <button
                type="button"
                onClick={() => setStoredFilter('all')}
              ...
              >
                All Properties ({inventory.length})
              </button>
              <button
                type="button"
                onClick={() => setStoredFilter('mine')}
                className={`px-3 py-1.5 rounded-lg font-medium transition ${
                  storedFilter === 'mine'
                    ? 'bg-emerald-600 text-white font-bold shadow-sm'
                    : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              ...
              >
                My Managed Listings ({inventory.filter(p => isAgentOwner(p)).length})
              </button>
            </div>

            {/* Quick Button to Switch back to Creator */}
            <button
              type="button"
              onClick={() => setActiveSection('create')}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs shadow-md shadow-emerald-600/20 transition"
            >
        ...
        {(() => {
          const filteredListings = inventory.filter(prop => {
            if (storedFilter === 'mine' && !isAgentOwner(prop)) return false;
            if (storedSearchQuery) {
              const query = storedSearchQuery.toLowerCase();
              const text = `${prop.address || ''} ${prop.propertyType || ''} ${prop.agentName || ''}`.toLowerCase();
          ...
          if (filteredListings.length === 0) {
            return (
              <div className="p-12 text-center rounded-3xl dark:bg-slate-900/40 bg-white border dark:border-slate-800 border-slate-200 space-y-3">
                <FolderOpen className="w-10 h-10 text-slate-400 mx-auto opacity-50" />
                <h3 className="text-base font-bold dark:text-white text-slate-900">No matching stored properties found</h3>
                <p className="text-xs text-slate-500 max-w-md mx-auto">
                  {storedFilter === 'mine' 
                    ? `You haven't uploaded any listings yet as ${currentUser?.name || 'this agent'}. Switch to "All Properties" or create a new listing.`
                    : 'Try clearing your search query or creating a new listing above.'}
                </p>
                <button
                  type="button"
                  onClick={() => setActiveSection('create')}
            ...
            <div className="space-y-6">
              {filteredListings.map((property) => {
                const photos = Array.isArray(property.photos) && property.photos.length > 0 
                  ? property.photos 
                  : (property.photoUrl ? [property.photoUrl] : ['https://images.unsplash.com/photo-1570129477492-45c003edd2be?auto=format&fit=crop&w=1200&q=80']);
                
                const activePhotoIdx = (storedPhotoIdxMap[property.id] || 0) % photos.length;
                const activePhoto = photos[activePhotoIdx] || photos[0];
                const isOwner = isAgentOwner(property);
                const isCover = activePhotoIdx === 0;

                return (
                  <div
                    key={property.id}
                    className="glass-panel rounded-3xl border dark:border-slate-800 border-slate-200 overflow-hidden shadow-lg bg-white dark:bg-slate-900/70 transition-all"
                  >
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
                      
                      {/* Left: Stored Photo Preview and Removal Controls (5 Cols) */}
                      <div className="lg:col-span-5 p-5 flex flex-col justify-between space-y-4 border-b lg:border-b-0 lg:border-r dark:border-slate-800 border-slate-200 bg-slate-50/50 dark:bg-slate-950/40">
                        <div className="space-y-2">
                          {/* Main Active Photo Container */}
                          <div className="relative w-full aspect-video sm:aspect-4/3 rounded-2xl overflow-hidden bg-slate-950 border dark:border-slate-800 border-slate-200 shadow-inner group">
                            <img
                              src={activePhoto}
                            ...
                            <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 z-10">
                              {isCover ? (
                                <span className="px-2.5 py-1 rounded-lg bg-amber-500 text-slate-950 text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 shadow">
                                  <Star className="w-3 h-3 fill-slate-950" /> Cover Photo
                                </span>
                              ) : (
                                <span className="px-2.5 py-1 rounded-lg bg-slate-950/80 text-white text-[10px] font-mono backdrop-blur-sm border border-white/10">
                                  Angle #{activePhotoIdx + 1}
                                </span>
                              )}
                            </div>

                            <div className="absolute top-2.5 right-2.5 px-2 py-1 rounded-lg bg-slate-950/80 text-white text-[10px] font-mono border border-white/10 flex items-center gap-1 z-10">
                            ...
                            {photos.length > 1 && (
                              <>
                                <button
                                  type="button"
                                  onClick={() => handleStoredPrevPhoto(property.id, photos.length)}
                                  className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-slate-950/80 hover:bg-slate-950 text-white flex items-center justify-center border border-white/20 transition-all z-10 hover:scale-110"
                                  title="Previous photo"
                                >
                                  <ChevronLeft className="w-4 h-4" />
                                </button>
                                <button
                                  type="button"
                                  onClick={() => handleStoredNextPhoto(property.id, photos.length)}
                                  className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-slate-950/80 hover:bg-slate-950 text-white flex items-center justify-center border border-white/20 transition-all z-10 hover:scale-110"
                                  title="Next photo"
                                >
                                  <ChevronRight className="w-4 h-4" />
                                </button>
                            ...
                            {/* Specific Agent Removal Action Button directly over image */}
                            <div className="absolute bottom-2.5 right-2.5 z-10">
                              {isOwner ? (
                                <button
                                  type="button"
                                  onClick={() => handleAttemptRemoveStoredPhoto(property, activePhotoIdx)}
                                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-red-600/90 hover:bg-red-600 text-white text-xs font-bold transition shadow-lg backdrop-blur-sm hover:scale-105"
                                  title="Authorized: Remove this photo"
                                >
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleAttemptRemoveStoredPhoto(property, activePhotoIdx)}
                                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900/90 hover:bg-slate-950 text-amber-400 text-xs font-semibold transition shadow-lg border border-amber-500/30 backdrop-blur-sm"
                                  title="Click to view permission requirements"
                                >
                                  <Lock className="w-3.5 h-3.5 text-amber-400" />
                                  <span>Locked (Owner Only)</span>
                                </button>
                              )}
                            </div>
                          </div>
                            ...
                            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 pt-1 custom-scrollbar">
                              {photos.map((p, idx) => (
                                <button
                                  key={idx}
                                  type="button"
                                  onClick={() => setStoredPhotoIdxMap(prev => ({ ...prev, [property.id]: idx }))}
                                  className={`relative w-12 h-10 rounded-lg overflow-hidden shrink-0 border-2 transition ${
                                    activePhotoIdx === idx
                        ...
                        {/* Add More Photos Row (Only if Owner) */}
                        {isOwner && (
                          <div className="pt-2 border-t dark:border-slate-800 border-slate-200">
                            <div className="flex items-center gap-1.5">
                              <input
                                type="url"
                                placeholder="Add image URL to this listing..."
                                value={newPhotoUrlMap[property.id] || ''}
                                onChange={(e) => setNewPhotoUrlMap(prev => ({ ...prev, [property.id]: e.target.value }))}
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter') {
                                    e.preventDefault();
                                    handleAttemptAddStoredPhoto(property);
                                  }
                                }}
                                className="flex-1 dark:bg-slate-900 bg-white dark:border-slate-700 border-slate-300 rounded-xl px-3 py-1.5 text-xs dark:text-white text-slate-900 font-mono focus:outline-none focus:border-emerald-500"
                              />
                              <button
                                type="button"
                                onClick={() => handleAttemptAddStoredPhoto(property)}
                                className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shrink-0 transition"
                              >
                                + Add
                              </button>
                            </div>
                      ...
                      {/* Right: Property Details & Ownership Status (7 Cols) */}
                      <div className="lg:col-span-7 p-6 flex flex-col justify-between space-y-5">
                        <div className="space-y-3">
                          {/* Header & Ownership Pill */}
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b dark:border-slate-800 border-slate-200 pb-3">
                            <div>
                              ...
                              <h3 className="text-base font-bold dark:text-white text-slate-900 flex items-center gap-1.5 mt-0.5">
                            ...
                            <div className="shrink-0">
                              {isOwner ? (
                                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-xs font-bold">
                                  <ShieldCheck className="w-3.5 h-3.5 text-emerald-500" />
                                  <span>Managed by You</span>
                                </div>
                              ) : (
                                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-700 dark:text-amber-400 text-xs font-medium">
                                  <Lock className="w-3.5 h-3.5 text-amber-500" />
                                  <span>Stored by {property.agentName || 'Agent'}</span>
                                </div>
                              )}
                            </div>
                          </div>

                          {/* Agent Security & Permission Explanation Box */}
                          <div className={`p-3 rounded-xl border text-xs flex items-start gap-2.5 ${
                            isOwner 
                              ? 'dark:bg-emerald-950/30 bg-emerald-50/70 dark:border-emerald-500/30 border-emerald-200 text-emerald-800 dark:text-emerald-300'
                              : 'dark:bg-amber-950/20 bg-amber-50/70 dark:border-amber-500/30 border-amber-200 text-amber-800 dark:text-amber-300'
                          }`}>
                            {isOwner ? (
                              <>
                                <Unlock className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                                <div>
                                  <span className="font-bold">Authorized Photo Control:</span> You are the verified agent for this listing. You have permission to add, remove, and update stored photos.
                                </div>
                              </>
                            ) : (
                              <>
                                ...
                                <div>
                                  <span className="font-bold">Photo Removal Protection:</span> Only <strong>{property.agentName || property.agentEmail || 'the original agent'}</strong> is authorized to remove photos from this listing. Your current session has read-only access.
                                </div>
                              </>
                            )}
                          </div>
                          ...
                          {/* Specifications */}
                          <div className="flex flex-wrap items-center gap-4 text-xs dark:text-slate-300 text-slate-700 pt-1">
                            ...
                            <span className="flex items-center gap-1">
                              <Maximize2 className="w-3.5 h-3.5 text-slate-400" /> {formatNumber(property.sqft)} sqft
                            </span>
                          </div>

                          {/* Key Features */}
                          {(property.keyFeatures || property.features) && (
                            <div className="flex flex-wrap gap-1.5 pt-1">
                              {(property.keyFeatures || property.features).slice(0, 5).map((f, i) => (
                                <span
                                  key={i}
                                  className="px-2 py-0.5 rounded-lg text-[11px] dark:bg-slate-800 bg-slate-100 dark:text-slate-300 text-slate-600 border dark:border-slate-700 border-slate-200"
                                >
                                  {f}
                                </span>
                              ))}
                        ...
                        <div className="pt-3 border-t dark:border-slate-800 border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                          <div className="text-slate-500 dark:text-slate-400">
                            Listing Agent: <strong className="dark:text-slate-200 text-slate-800">{property.agentName || 'Vikram Malhotra'}</strong>
                            {property.agency && <span className="opacity-70"> ({property.agency})</span>}
                          </div>
                          ...
                          {isOwner && (
                            <button
                              type="button"
                              onClick={() => {
                                if (window.confirm(`Delete entire listing for "${property.address}"?`)) {
                                  onDeleteListing?.(property.id);
                                }
                              }}
                              className="text-red-500 hover:text-red-600 hover:underline font-semibold text-[11px] self-start sm:self-auto"
                            >
    ...
    {/* Specific Agent Permission Alert Modal */}
    {permissionAlert && (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl">
          <div className="flex items-center gap-3 text-amber-500">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                {permissionAlert.title || 'Action Blocked'}
              </h3>
          ...
          <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-950 border dark:border-slate-800 border-slate-200 text-xs space-y-1.5">
            <div className="flex justify-between">
              <span className="text-slate-500">Authorized Agent:</span>
              <strong className="text-emerald-600 dark:text-emerald-400">{permissionAlert.authorizedAgent}</strong>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">Active Profile:</span>
              <strong className="text-slate-800 dark:text-slate-200">{currentUser?.name || 'Guest'} ({currentUser?.role || 'user'})</strong>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 pt-2">
            <button
              ...
              className="flex-1 py-2.5 px-4 rounded-xl bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-400 hover:to-indigo-500 text-white font-bold text-xs transition shadow-md shadow-indigo-500/25"
            >
            ...
            <button
              ...
              className="py-2.5 px-4 rounded-xl border dark:border-slate-700 border-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 text-xs font-semibold dark:text-slate-300 text-slate-700 transition"
            >
              Dismiss
            </button>
          </div>
